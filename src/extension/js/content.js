...
<div class="subtitles-streaming-view">
  <div class="main-frame1">
    <div class="subtitle-text">
      ...
      <div class="copy" title="Copy text">
      ...
      <div class="toggle-container" title="Toggle Transcript or Translate">
...
`
var check_status_transcript = true
...
function dragInit(elemToDrag, elemHeight) {
  let pos3 = 0, pos4 = 0;
  ...
  elemToDrag.onmousedown = function (e) {
    ...
    document.onmousemove = function (e) {
      ...
      if (isResizing) {
        ...
        pos3 = e.clientX;
        pos4 = e.clientY;
        elemToDrag.style.top = (e.clientY - offsetY) + "px";
        elemToDrag.style.left = (e.clientX - offsetX) + "px";
      }
    };
  };
}
  ...
  navigator.mediaDevices.getDisplayMedia({ video: true, audio: true, preferCurrentTab: true }).then(stream => {
    ...
    recorder.addEventListener('dataavailable', async evt => {
      ...
      if (evt.data.size > 0 && recorder.state === "recording" && isRecording === true) {
        ...
        formData.append("st", check_status_transcript);
...
function waitForSubtitles() {
  ...
  if (wrapper) {
    ...
    const copy = streamingCaptionRoot.querySelector('.copy')
    ...
    const ontranslate = streamingCaptionRoot.querySelector(".toggle-container");
    ontranslate.addEventListener("click", function () {
      console.log("================")
      const toggleInput = ontranslate.querySelector('#toggle');
      // console.log(toggleInput)
      toggleInput.checked = !toggleInput.checked;
      if (toggleInput.checked) {
        check_status_transcript = false
        // ontranslate.querySelector('.toggle-label').textContent = 'On';
        console.log('Translate mode enabled');
        // Add your code here for when the toggle is enabled
      } else {
        check_status_transcript = true
        // ontranslate.querySelector('.toggle-label').textContent = 'Off';
        console.log('Transcript mode enabled');
        // Add your code here for when the toggle is disabled
      }
    })
    ...
    window.onload = function () {
      ...
      dragInit(subtitlesStreamingView, subText);
      defaultSize(defaultItem, subtitlesStreamingView, subText);
    };
    // Kiểm tra đã đăng nhập hay chưa - service-worker
    chrome.storage.local.get(['token'], function (result) {
      if (result.token != 'None' && result.token != null && result.token != '') {
    ...
    subBtnRight.addEventListener("click", async function () {
      ...
      if (status) {
        ...
        console.log("Chuyển hướng đến một đường link khác");
